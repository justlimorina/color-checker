// Curated bestOf color list from meodai/color-names (color-name-list)
...
// Pre-calculate RGB values for faster matching
const parsedColors = colorNames.map(color => {
    ...
    return { name: color.name, hex: color.hex.toUpperCase(), r, g, b };
});

/**
 * Finds the nearest color name from the list using Euclidean distance squared in RGB space.
 * @param {string} hex - The hex color code (e.g. "6750A4" or "#6750A4")
 * @returns {{name: string, hex: string, r: number, g: number, b: number}}
 */
export function getNearestColor(hex) {
    const cleanHex = hex.startsWith('#') ? hex.substring(1) : hex;
    if (cleanHex.length !== 6) return { name: "Unknown", hex: "#000000", r: 0, g: 0, b: 0 };
    ...
    if (isNaN(r) || isNaN(g) || isNaN(b)) {
        return { name: "Unknown", hex: "#000000", r: 0, g: 0, b: 0 };
    }
    ...
    for (let i = 0; i < parsedColors.length; i++) {
        const c = parsedColors[i];
        
        // Simple Euclidean distance squared in RGB space
        const dr = r - c.r;
        const dg = g - c.g;
        const db = b - c.b;
    ...
    return nearest || { name: "Unknown", hex: "#000000", r: 0, g: 0, b: 0 };
}
